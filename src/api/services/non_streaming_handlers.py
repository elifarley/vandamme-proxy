...
class NonStreamingHandler(ABC):
    ...
    async def handle_non_streaming_request(
        ...
        *,
        ...
        """Handle a non-streaming request and return the response.
        ...
        Args:
            ...
            tool_use_count: Number of tool uses in request.
            tool_result_count: Number of tool results in request.
    ...
    async def handle_non_streaming_request(
        ...
        *,
        ...
        if hasattr(config.provider_manager, "middleware_chain"):
            ...
            processed_response = await config.provider_manager.middleware_chain.process_response(
                response_context
            )
            anthropic_response = processed_response.response

        # Update metrics
        if config.log_request_metrics and metrics:
    ...
    async def handle_non_streaming_request(
        ...
        *,
        ...
        if config.log_request_metrics and metrics:
            ...
            metrics.cache_creation_tokens = usage.get("cache_creation_tokens", 0) if usage else 0
